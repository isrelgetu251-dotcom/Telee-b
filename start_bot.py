...
def check_dependencies():
    ...
    critical_packages = [
        'telegram', 'python-dotenv', 'schedule'
    ]
...
def check_configuration():
    ...
    if not BOT_TOKEN or "YOUR_" in BOT_TOKEN:
        issues.append("❌ BOT_TOKEN is not properly set in .env file")
    
    if not CHANNEL_ID:
        issues.append("❌ CHANNEL_ID is not set in .env file")
    
    if not ADMIN_IDS:
        issues.append("❌ No admin IDs configured in .env file")
